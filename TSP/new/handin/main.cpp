...
void update(double* phrom,int *path,double ro,int min,int dim,double maxP,double minP){
        double* tmp;
        tmp=new double [dim*dim];
        for(int i=0;i!=dim;i++)
                for(int j=0;j!=dim;j++)
...
/*		cout<<"solve:";
		...
		if(atoi(argv[3])==1){
			update(phrom,solution[pos],ro,min,dim,0.6,0.000000003);
		}else{
			update(phrom,solution[pos],ro,min,dim);//0.6,0.000000003);
		}
		if(L-min==0){
