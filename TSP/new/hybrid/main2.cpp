...
double ACO(double* phrom,double* dis,int dim,int ro,int cons,double maxSum,int* staticSolve){
	P* prob;
	prob=new P [(dim-1)];
	vector<int*> solution;
	bool flag=true;
...
//	printM(dis,dim);
	...
	L=ACO(phrom,dis,dim,ro,cons,maxSum,staticSolve);	
