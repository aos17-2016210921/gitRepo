...
void cross(int* p1,int* p2,int dim,int* off){//from 1 to dim+1
	int* count;
	int* unused;
	count=new int[dim];
	unused=new int[dim];
			...
			while(conti){
				...
				int i=0;
						...
						deadNum++;	
                        	...
                        	double var=0.5;
				double cross=0.2;
				int popul=gene.size()*cross;
				
				for(int i=0;i!=popul;i=i+2){
					cross(gene[i]+1,gene[i+1]+1,dim-1,gene[deadList[deadNum--]]);
				}
				...
				for(int i=0;i!=dim-1;i++){
					length[i]=0;
					for(int j=1;j!=dim;j++){
						length[i]+=dis[gene[i][j-1]*dim+gene[i][j]];
					}
					length[i]+=dis[gen[i][dim-1]*dim];
				}
				int min=localMin;
				for(int i=0;i!=dim-1;i++){
					if(min>length[i]){
